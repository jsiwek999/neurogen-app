...
// Next.js 15: searchParams is a Promise in PageProps
type Props = { searchParams?: Promise<{ token?: string }> };

async function count(where?: (q: any) => any) {
  let q = supabaseService.from('newsletter_subscribers').select('*', {
    count: 'exact',
    head: true,
  });
  if (where) q = where(q);
...
export default async function Page({ searchParams }: Props) {
  ...
  const ok = sp.token && sp.token === process.env.ADMIN_METRICS_TOKEN;
    ...
    return (
      <div className="p-6">
        ...
        <p className="text-sm mt-2 text-red-500">
          Unauthorized. Append <code>?token=…</code>.
        </p>
      </div>
    );
  }
