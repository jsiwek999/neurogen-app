...
export async function generateStaticParams(){ return Object.keys(META).map(slug=>({ slug })); }
export default function RitualPage({ params }: { params:{ slug:string } }){
  const cfg = META[params.slug]; if(!cfg) return notFound();
  return (
    <main className="mx-auto max-w-2xl space-y-6">
      <h1 className="text-2xl font-semibold text-white">{cfg.title}</h1>
      <pre className="rounded-2xl border border-white/10 bg-white/5 p-5 text-white/80 whitespace-pre-wrap">{cfg.emx}</pre>
    </main>
  );
}
